...
def fast_pkt_info(buf,ts):
    ...
    if isinstance(ip.data, dpkt.udp.UDP):  # 解包，判断传输层协议是否是UDP
        ...
        payload = transf_data.data
        payload_vector_list = []
        for i in range(max_payload_len):
            payload_vector_list.append(-1)
        for i in range(min(max_payload_len,len(payload))):
            payload_vector_list[i] = payload[i]
    ...
    data.extend(payload_vector_list)
